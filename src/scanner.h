/*
 * MIT License
 * Copyright (c) 2025 IMSDcrueoft (https://github.com/IMSDcrueoft)
 * See LICENSE file in the root directory for full license text.
*/
#pragma once
#include "common.h"

typedef enum {
	// Single-character tokens. 
	TOKEN_LEFT_PAREN, TOKEN_RIGHT_PAREN,
	TOKEN_LEFT_BRACE, TOKEN_RIGHT_BRACE,
	TOKEN_LEFT_SQUARE_BRACKET, TOKEN_RIGHT_SQUARE_BRACKET,
	TOKEN_COMMA, TOKEN_DOT, TOKEN_MINUS, TOKEN_PLUS,
	TOKEN_SEMICOLON, TOKEN_SLASH, TOKEN_STAR, TOKEN_PERCENT, TOKEN_COLON,
	// Bit-calc
	TOKEN_BIT_AND, TOKEN_BIT_OR, TOKEN_BIT_XOR, TOKEN_BIT_NOT, TOKEN_BIT_SHL, TOKEN_BIT_SHR, TOKEN_BIT_SAR,
	// One or two character tokens. 
	TOKEN_BANG, TOKEN_BANG_EQUAL,
	TOKEN_EQUAL, TOKEN_EQUAL_EQUAL,
	TOKEN_GREATER, TOKEN_GREATER_EQUAL,
	TOKEN_LESS, TOKEN_LESS_EQUAL,
	// Literals. 
	TOKEN_IDENTIFIER, TOKEN_STRING, TOKEN_STRING_ESCAPE, TOKEN_NUMBER, TOKEN_NUMBER_BIN, TOKEN_NUMBER_HEX,
	// Builtin Literals. 
	TOKEN_MODULE_MATH, TOKEN_MODULE_ARRAY, TOKEN_MODULE_STRING, TOKEN_MODULE_TIME, TOKEN_MODULE_SYSTEM,
	// Keywords. 
	TOKEN_AND, TOKEN_CLASS, TOKEN_FALSE,
	TOKEN_FOR, TOKEN_FUN, TOKEN_NIL, TOKEN_OR,
	TOKEN_RETURN, TOKEN_THIS,
	TOKEN_TRUE, TOKEN_VAR,
	TOKEN_BREAK, TOKEN_CONTINUE, TOKEN_LAMBDA,
	TOKEN_BRANCH, TOKEN_NONE, TOKEN_INSTANCE_OF, TOKEN_TYPE_OF,

	TOKEN_ERROR, TOKEN_EOF, TOKEN_IGNORE, TOKEN_UNTERMINATED_COMMENT,
} TokenType;

typedef struct {
	C_STR start;
	C_STR current;
	uint32_t line;
} Scanner;

typedef struct {
	C_STR start;
	TokenType type;
	uint32_t length;
	uint32_t line;
} Token;

void scanner_init(C_STR source);

Token scanToken();